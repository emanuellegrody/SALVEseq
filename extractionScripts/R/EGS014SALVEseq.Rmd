...
output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/expressionDF/rawcounts/"
...
for (i in 1:nrow(samples)) {
  ...
  tryCatch({
    ...
    singlecell_df <- targetExpressionDF(seurat_obj, "mac239", counts_type = "raw")
...
```{r}
SALVE_isoforms <- isoforms_deconvolve(SALVE_combined)
output.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/counts/expressionDF/"
...
for (i in 1:nrow(samples)) {
  ...
  tryCatch({
    SALVE <- SALVE_isoforms %>%
      ...
      mutate(SALVE = rowSums(across(c(US, spliced, S, SS, MS, any)), na.rm = TRUE)) %>%
      filter(SALVE != min(SALVE, na.rm = TRUE)) %>%
      select(cellID, SALVE)
...
for (sample_name in names(joint_data)) {
  ...
  p <- ggplot(data = both, aes(x = SALVE, y = GEX)) +
      geom_point() +
      labs(
        x = "SALVE",
        y = "GEX",
