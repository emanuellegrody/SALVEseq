...
### Current strategy: bamsort (UPDATE)
```{r}
samples <- data.frame(
  datasets = c(
    "JK85_D13",
...
output.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/bamsort/reads/20250518/"
...
for (i in 1:nrow(samples)) {
  ...
  is_relevant <- sapply(filenames, is_v4bamsort_file)
...
convert_umi_to_salve <- function(umi_file, output_file, min_reads = 1, min_region_count = 1) {
  ...
  if (min_reads > 0) {
    ...
    #cat("Found", length(valid_cells), "cells with at least", min_reads, "reads\n")
...
is_v4bamsort_file <- function(filename) {
...
output.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250520"
  ...
  if (nrow(all_data) > 0) {
    ...
    tryCatch({
      ...
      if (sample %in% names(raw_cellIDs) && !is.null(raw_cellIDs[[sample]])) {
        ...
        filtered_umi_read_counts <- resolve_multimap(valid_umi_read_counts, "SALVE")
...
# Example paths
input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250520"
output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250520/converted/min1/nocountLTR"

#input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/bamsort/reads"
#output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/bamsort/reads/converted/min2"

# Process all samples
process_all_samples_umi_to_salve(samples, input_dir, output_dir, min_reads = 3, min_region_count = 1)
...
for (sample_name in names(joint_data)) {
  ...
  current_df <- joint_data[[sample_name]] %>%
    mutate(vgenes = (D1_US != 0) + (D1_S != 0) + (tat_S != 0) + (tat_US != 0) + (nef != 0))
