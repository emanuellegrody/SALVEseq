...
identify_file_type <- function(filename) {
...
for (i in 1:nrow(samples)) {
  ...
  for (j in seq_along(filenames)) {
    ...
    # Use the identify_file_type function to get the key
    file_type <- identify_file_type(filenames[j])
    keys[j] <- file_type
...
Read in and filter each file separately
```{r}
samples <- data.frame(
  datasets = c(
...
```{r}
# Function to detect if a file belongs to a specific category
is_total_lessLTR_file <- function(filename) {
...
input.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/alignment/"
output.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250518/"
...
for (i in 1:nrow(samples)) {
  ...
  filenames <- basename(sample_files)
  is_relevant <- sapply(filenames, is_total_lessLTR_file)
  relevant_files <- sample_files[is_relevant]
  relevant_filenames <- filenames[is_relevant]
  ...
  categories <- sapply(relevant_filenames, extract_category)
  
  # Create a data frame to store the combined results
  all_data <- data.frame()
  
  # Process each relevant file
  for (j in seq_along(relevant_files)) {
    extracted_data <- NULL  # Initialize as NULL for each file
    
    tryCatch({
      # Read the CSV file, expecting standard column names
      file_data <- fread(relevant_files[j], data.table = FALSE)
      
      # Check if file has content
      if(nrow(file_data) == 0) {
        cat("Warning: File is empty:", relevant_filenames[j], "\n")
        next
      }
      ...
      cat("Error reading file:", relevant_filenames[j], "\n")
      cat("Error message:", conditionMessage(e), "\n")
      
      # Try reading without header
      tryCatch({
        cat("Attempting to read without header...\n")
        file_data <- fread(relevant_files[j], data.table = FALSE, header = FALSE)
        
        # If we got here, we were able to read the file without a header
        # Assign column names based on position
        if(ncol(file_data) >= 3) {
          colnames(file_data) <- c("cellID", "UMI", "read", rep("extra", ncol(file_data) - 3))
          
          # Create extracted data
          extracted_data <- file_data %>%
            select(cellID, UMI, read) %>%
            mutate(category = categories[j])
            
          cat("Successfully read file without header\n")
        } else {
          cat("File has fewer than 3 columns, cannot process:", ncol(file_data), "columns found\n")
        }
      }, error = function(e2) {
        cat("Still failed to read file. Final error:", conditionMessage(e2), "\n")
      })
    })
  ...
  if (nrow(all_data) > 0) {
    ...
    # Count reads per UMI - no need to count distinct reads since each row represents a count
    # read column originally contained count values
    tryCatch({
      ...
      # Combine the cell-level and category-level data
      ...
      if (sample %in% names(raw_cellIDs) && !is.null(raw_cellIDs[[sample]])) {
        ...
        # Filter the cell-level summary data
      ...
      # Create a visualization dataset for plotting (use filtered data)
...
Testing: filter reads on full v filtered on BAM
```{r}
input.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250518/"
D13 <- fread(paste0(input.dir, "D13_UMI_read_counts_full.csv"))
filtered <- D13[D13$read_count > 1]


uninfected <- fread(paste0(input.dir, "Uninfected_UMI_read_counts_full.csv"))

```


Testing: how many UMI are in multiple categories?
```{r}
# Function to analyze UMI categories across samples
analyze_umi_categories <- function(dir_path, sample_names) {
  results <- list()
  
  for (sample in sample_names) {
    # Construct file path for this sample
    file_path <- file.path(dir_path, paste0(sample, "_UMI_read_counts.csv"))
    ...
    if (!all(c("cellID", "UMI", "category") %in% colnames(data))) {
      cat("Missing required columns in", file_path, "\n")
      next
    }
    
    # Count unique cells
    num_cells <- length(unique(data$cellID))
    cat("Number of unique cells:", num_cells, "\n")
    
    # Count unique UMIs
    num_umis <- length(unique(paste(data$cellID, data$UMI, sep = "_")))
    cat("Number of unique cell-UMI pairs:", num_umis, "\n")
    
    # Group by cell and UMI, count categories
    library(dplyr)
    
    # Count categories per UMI per cell
    umi_category_counts <- data %>%
      group_by(cellID, UMI) %>%
      summarize(
    ...
    # Count UMIs with multiple categories
    multi_category_umis <- umi_category_counts %>%
      filter(num_categories > 1)
    
    # Count cells with multi-category UMIs
    cells_with_multi_category_umis <- length(unique(multi_category_umis$cellID))
    
    # Get distribution of category counts
    category_distribution <- table(umi_category_counts$num_categories)
    
    # Count most common category combinations
    if (nrow(multi_category_umis) > 0) {
      category_combinations <- table(multi_category_umis$categories)
      top_combinations <- sort(category_combinations, decreasing = TRUE)[1:min(5, length(category_combinations))]
    } else {
      top_combinations <- "No multi-category UMIs found"
    }
    
    # Calculate percent of UMIs with multiple categories
    percent_multi <- (nrow(multi_category_umis) / nrow(umi_category_counts)) * 100
    
    # Store results for this sample
    results[[sample]] <- list(
      total_cells = num_cells,
      total_umis = num_umis,
      multi_category_umis = nrow(multi_category_umis),
      percent_multi_category = percent_multi,
      cells_with_multi_category = cells_with_multi_category_umis,
      percent_cells_with_multi = (cells_with_multi_category_umis / num_cells) * 100,
    ...
    # Print summary
    cat("\nSummary for sample:", sample, "\n")
    cat("Total cells:", num_cells, "\n")
    cat("Total UMIs:", num_umis, "\n")
    cat("UMIs with multiple categories:", nrow(multi_category_umis), 
        sprintf("(%.2f%%)", percent_multi), "\n")
    cat("Cells with at least one multi-category UMI:", cells_with_multi_category_umis,
        sprintf("(%.2f%%)", (cells_with_multi_category_umis / num_cells) * 100), "\n")
    
    cat("\nDistribution of category counts per UMI:\n")
    ...
    if (nrow(multi_category_umis) > 0) {
      ...
      # Optional: Find cells with the most multi-category UMIs
      cells_multi_count <- multi_category_umis %>%
        group_by(cellID) %>%
        summarize(count = n(), .groups = "drop") %>%
        arrange(desc(count))
      
      cat("\nTop 5 cells with most multi-category UMIs:\n")
      print(head(cells_multi_count, 5))
    }
...
# Example usage:
output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads"

# Run the analysis
results <- analyze_umi_categories(output_dir, samples$samples)

# Optional: Create a summary table across all samples
summary_df <- data.frame(
  sample = names(results),
  total_cells = sapply(results, function(x) x$total_cells),
  total_umis = sapply(results, function(x) x$total_umis),
  multi_category_umis = sapply(results, function(x) x$multi_category_umis),
  percent_multi = sapply(results, function(x) x$percent_multi_category),
  cells_with_multi = sapply(results, function(x) x$cells_with_multi_category),
  percent_cells_multi = sapply(results, function(x) x$percent_cells_with_multi)
)

# Write summary table to a file
write.csv(summary_df, file.path(output_dir, "multi_category_umi_summary.csv"), row.names = FALSE)
...
### Setting minimums
```{r}
convert_umi_to_salve <- function(umi_file, output_file, min_reads = 1, min_region_count = 1) {
  # Read the UMI data
  ...
  if (min_reads > 0) {
    # Aggregate reads by cellID
    cell_counts <- aggregate(read_count ~ cellID, data = umi_data, FUN = sum)
    
    # Filter cells with at least min_reads
    valid_cells <- cell_counts$cellID[cell_counts$read_count >= min_reads]
  ...
  categories <- unique(umi_data$category)
  
  # Get unique cell IDs
  cell_ids <- unique(umi_data$cellID)
  
  # Initialize empty SALVE data frame with all categories as columns
  salve_data <- data.frame(cellID = cell_ids)
  ...
  # Get all categories
  categories <- colnames(umi_counts_df)[colnames(umi_counts_df) != "cellID"]
  
  # Initialize category columns with zeros
  for (cat in categories) {
    cat_col_name <- make.names(cat)
    if (cat_col_name != cat) {
      cat("Warning: Modified category name from", cat, "to", cat_col_name, "for R compatibility\n")
    }
    salve_data[[cat_col_name]] <- 0
  }
  
  # Fill in the SALVE data with UMI counts
  for (i in 1:nrow(umi_counts_df)) {
    ...
    if (length(row_idx) == 1) {
      # For each category, set the UMI count
      for (cat in categories) {
        cat_col_name <- make.names(cat)
        salve_data[row_idx, cat_col_name] <- umi_counts_df[i, cat]
      }
    } else {
  ...
  valid_cells <- salve_data$cellID[salve_data$region_count >= min_region_count]
  filtered_salve_data <- salve_data[salve_data$cellID %in% valid_cells, ]
  
  # Remove the helper column
  filtered_salve_data$region_count <- NULL
  
  # # Calculate total_lessLTR (sum of all categories except LTR)
  # ltr_col <- if ("LTR" %in% categories) "LTR" else NULL
  # category_cols <- setdiff(categories, ltr_col)
  # 
  # salve_data$total_lessLTR <- 0
  # for (cat in category_cols) {
  #   salve_data$total_lessLTR <- salve_data$total_lessLTR + salve_data[[cat]]
  # }
  # 
  # # Ensure columns are in the right order
  # # Start with cellID, then categories, then total_lessLTR
  # desired_order <- c("cellID", setdiff(colnames(filtered_salve_data), c("cellID", "total_lessLTR")), "total_lessLTR")
  # filtered_salve_data <- filtered_salve_data[, desired_order]
  
  # Write the output file
  write.csv(filtered_salve_data, output_file, row.names = FALSE)
...
process_all_samples_umi_to_salve <- function(sample_list, input_dir, output_dir, min_reads = 1, min_region_count = 1) {
  ...
  for (sample in sample_list) {
    ...
    tryCatch({
      salve_data <- convert_umi_to_salve(input_file, output_file, min_reads, min_region_count)
  ...
  cat("\nConversion summary:\n")
  cat("  Successful conversions:", successful, "\n")
  cat("  Failed conversions:", failed, "\n")
  cat("All samples processed.\n")
}

# Example usage
#umi_file <- "D195_UMI_read_counts.csv"
#output_file <- "D195_SALVE_filtered.csv"
#salve_data <- convert_umi_to_salve(umi_file, output_file, min_reads = 0)

# Verify conversion with original SALVE file
# verify_conversion(umi_file, "D195_SALVE_full.csv")

# Example to process all samples
samples <- c(
  "D13",
  "D195",
  "D83",
...
# Example paths
input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250518"
output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/SALVE/bamsort/reads/20250518/converted/min2/nocountLTR"

#input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/bamsort/reads"
#output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/bamsort/reads/converted/min2"

# Process all samples
process_all_samples_umi_to_salve(samples, input_dir, output_dir, min_reads = 2, min_region_count = 2)

```



### Older: compare bamsort to mkcounts
```{r}
GEX_mkcounts <- read.csv("/projects/b1042/GoyalLab/egrody/extractedData/EGS004/singleCell/counts/Mmul_10_mac239/expressionDF/JK85_D13_Mmulmac_mac239.csv", row.names = "X")
GEX_bamsort <- read.csv("/projects/b1042/GoyalLab/egrody/extractedData/EGS004/singleCell/bamsort/split/JK85_D13_bamsort_split_inside.csv")

GEX_mkcounts_minimal <- GEX_mkcounts %>%
  filter(mac239 > 0) %>%
  select(cellID, mac239)
GEX_bamsort_clean <- GEX_bamsort %>%
  #filter(reads > 1) %>%
  count(cellID) %>% rename(UMI = n) #%>%
  filter(UMI > 1)
cat(
  "Cells in both bamsort and mkcounts: \t",
...
full_GEX <- full_join(GEX_bamsort_clean, GEX_mkcounts_minimal, by = join_by(cellID)) %>%
  mutate(bamsort = log1p(UMI)) %>% rename(mkcounts = mac239)
full_GEX[is.na(full_GEX)] <- 0

max_value <- max(max(full_GEX$mkcounts, na.rm = TRUE), 
                 max(full_GEX$bamsort, na.rm = TRUE))

full_GEX_filter <- full_GEX %>%
  filter(!(bamsort > 0 & mkcounts == 0))

ggplot(full_GEX_filter, aes(x = mkcounts, y = bamsort)) + 
  geom_point() + 
  theme_minimal() +
  coord_fixed(ratio = 1) + 
  scale_x_continuous(limits = c(0, max_value)) + 
  scale_y_continuous(limits = c(0, max_value)) +
  labs(title= "D13 mac239 counts from two sources")

cat(
    "Correlation between bamsort and mkcounts (read and UMI filter):",
    round(cor(full_GEX$bamsort, full_GEX$mkcounts, method="pearson"), 3))
...
input.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS014/singleCell/bamsort/extracted"
...
for (sample_name in names(joint_data)) {
  ...
  current_df <- joint_data[[sample_name]] %>%
    mutate(vgenes = (D1_US != 0) + (tat_S != 0) + (nef != 0))
  
  # Create the subsets based on mac239 and total columns
  both <- current_df %>% filter(mac239 != 0 & vgenes > 2)
