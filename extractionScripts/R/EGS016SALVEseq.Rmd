...
output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS016/SALVE/bamsort/reads/KLRB1/minimum/" dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
...
for (i in 1:nrow(samples)) {
  ...
  tryCatch({
    seurat_obj <- SeuratPipeline(input.dir, sample_name, 
                                 output_dir = "/projects/b1042/GoyalLab/egrody/extractedData/EGS016/singleCell/Seurat/", 
                                 plots = TRUE, rds = TRUE)
...
cd8_expression <- GetAssayData(seurat_obj, layer = "data")["CD8A", ]
...
cd8a_expression <- GetAssayData(seurat_obj, layer = "data")["CD8A", ]
...
for (sample in samples) {
  ...
  expr1 <- as.matrix(GetAssayData(seurat_obj[, cells_set1], layer = "data")[var_features, ])
  expr2 <- as.matrix(GetAssayData(seurat_obj[, cells_set2], layer = "data")[var_features, ])
  expr_both <- as.matrix(GetAssayData(seurat_obj[, cells_both], layer = "data")[var_features, ])
  ...
  for (i in 1:n_iterations) {
    ...
    expr_random <- as.matrix(
      GetAssayData(seurat_obj[, random_cells], layer = "data")[var_features, ])
...
for (sample in samples) {
  ...
  expr_gex <- as.matrix(GetAssayData(seurat_obj[, cells_gex], layer = "data")[var_features, ])
  expr_salve <- as.matrix(GetAssayData(seurat_obj[, cells_salve], layer = "data")[var_features, ])
  expr_both <- as.matrix(GetAssayData(seurat_obj[, cells_both], layer = "data")[var_features, ])
...
subset_memory_cd4 <- function(seurat_obj, min_cd4_expr = 0.5) {
  cd4_expr <- GetAssayData(seurat_obj, layer = "data")["CD4", ]
  il7r_expr <- GetAssayData(seurat_obj, layer = "data")["IL7R", ]
...
test_metabolic_correlations <- function(joint_salve_data, joint_rds, 
  ...
  for (sample_name in names(joint_salve_data)) {
    ...
    if (!"total_UMI" %in% colnames(seurat_obj@meta.data)) {
      seurat_obj$total_UMI <- colSums(GetAssayData(seurat_obj, layer = "counts"))
      seurat_obj$total_genes <- colSums(GetAssayData(seurat_obj, layer = "counts") > 0)
    }
...
for (sample in subsets) {
  ...
  union <- gex_detected + salve_detected - intersection  # A + B - (A ∩ B)
