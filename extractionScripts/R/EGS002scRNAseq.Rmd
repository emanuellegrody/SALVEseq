...
```{r KLRB1}
subsets <- c("W0_concat_150M", "W0_concat_200M", "W0_concat_300M", "W0_concat_400M")

# Process
input.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS002/singleCell/bamsort/alignment/concat_subsample/"
output.dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS002/singleCell/bamsort/reads/concat_subsample/"

process_bamsortv6(subsets, input.dir, output.dir)

# Minimums
input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS002/singleCell/bamsort/reads/concat_subsample/"
output_dir <- paste0(input_dir, "minimum/")

process_all_set_minimumsv6("KLRB1", subsets, input_dir, TRUE, output_dir, min_reads_cell = 2)
```
...
# Input directories
salve_input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS016/SALVE/bamsort/reads/KLRB1/minimum/"
gex_input_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS002/singleCell/bamsort/reads/concat_subsample/minimum/"
umap_file <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS016/singleCell/Seurat/UMAPcoords/W0UMAP_coords.csv"

# Output directories
output_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS002/joint/dataframes/"
plot_dir <- "/projects/b1042/GoyalLab/egrody/extractedData/EGS002/joint/plots/"

# Create output directories if they don't exist
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(plot_dir, showWarnings = FALSE, recursive = TRUE)

# Load SALVE data (reference dataset)
W0_min <- read.csv(paste0(salve_input_dir, "SALVE_KLRB1_W0_SALVE_minreads5.csv"), row.names = "X")
...
GEX_KLRB1_W0_UMAP <- read.csv(umap_file, row.names = "X")
...
cat("===========================================\n")
cat("Sample\t\tTotal_cells\tboth\t10X_only\tSALVE_only\tPearson_r\tSpearman_rho\n")

# Loop through each GEX subsample
for (sample in subsets) {
  
  # Load GEX data for this subsample
  gex_file <- paste0(gex_input_dir, sample, "_KLRB1_filtered.csv")
  ...
  GEX_KLRB1_W0 <- read.csv(gex_file, check.names = FALSE)
  if (colnames(GEX_KLRB1_W0)[1] %in% c("X", "", "Unnamed: 0")) {
    rownames(GEX_KLRB1_W0) <- GEX_KLRB1_W0[,1]
    GEX_KLRB1_W0 <- GEX_KLRB1_W0[,-1]
  }
  ...
  if (nrow(both) > 2) {
    pearson_r <- cor(both$GEX, both$SALVE, method = "pearson")
    spearman_rho <- cor(both$GEX, both$SALVE, method = "spearman")
  } else {
    pearson_r <- NA
    spearman_rho <- NA
  }
  ...
  plotUMAP(W0_joint, colorby = GEX,
           paste0("W0 joint KLRB1: GEX (", sample, ")"),
           plot_dir,
           paste0("KLRB1_W0_GEX_", sample, ".pdf"),
           comparison = FALSE, color = "gray25")
  
  # Save dataframes for this subsample
  write.csv(both, paste0(output_dir, "KLRB1_both_", sample, ".csv"), row.names = FALSE)
  write.csv(onlySingleCell, paste0(output_dir, "KLRB1_onlySingleCell_", sample, ".csv"), row.names = FALSE)
  write.csv(onlySALVE, paste0(output_dir, "KLRB1_onlySALVE_", sample, ".csv"), row.names = FALSE)
  write.csv(W0_joint, paste0(output_dir, "KLRB1_all_", sample, ".csv"), row.names = FALSE)
}
plotUMAP(W0_joint, colorby = SALVE,
           paste0("W0 joint KLRB1: SALVE (", sample, ")"),
           plot_dir,
           paste0("KLRB1_W0_SALVE.pdf"),
           comparison = FALSE, color = "gray25")
```
###Correlations
```{r}
