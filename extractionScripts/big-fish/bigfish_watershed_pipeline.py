...
Usage:
    python -u bigfish_pipeline.py --input_dir /path/to/input --output_dir /path/to/output --channels Red Gold70 FarRed
"""
...
from skimage.morphology import remove_small_objects, binary_dilation, disk, closing, erosion, binary_erosion, opening
...
def segment_cells_by_intensity(cytoplasm_mip, nuc_label=None, min_cell_area=4000,
    ...
    """
    Watershed-based cell segmentation that separates touching cells.
    """
    import bigfish.multistack as multistack
    ...
    n_bright = len(np.unique(label(bright_mask))) - 1
    print("  Stage 1: {} extremely bright region(s) detected".format(n_bright))

    # Stage 2: Local contrast detection
    ...
    # Stage 3: Watershed-based separation
    print("  Stage 3 (watershed): separating touching cells...")

    opened_mask = opening(combined_mask, disk(3))
    distance = distance_transform_edt(opened_mask)

    # Determine seeds
    if nuc_label is not None and nuc_label.max() > 0:
        print("    Using {} nuclei as watershed seeds".format(nuc_label.max()))
        markers = nuc_label.copy()

        coords = peak_local_max(distance, min_distance=watershed_min_distance,
                                labels=opened_mask, exclude_border=False)

        distance_markers = np.zeros(distance.shape, dtype=int)
        for i, coord in enumerate(coords, start=markers.max() + 1):
            if markers[coord[0], coord[1]] == 0:
                distance_markers[coord[0], coord[1]] = i

        distance_markers = binary_dilation(distance_markers > 0, disk(2))
        distance_markers = label(distance_markers)

        combined_markers = markers.copy()
        max_nuc_label = markers.max()
        for i in range(1, distance_markers.max() + 1):
            mask = distance_markers == i
            if np.sum(combined_markers[mask] > 0) == 0:
                combined_markers[mask] = max_nuc_label + i

        markers = combined_markers
        print("    Total seeds: {} (nuclei + distance-based)".format(markers.max()))
    else:
        coords = peak_local_max(distance, min_distance=watershed_min_distance,
                                labels=opened_mask, exclude_border=False)

        print("    Using {} distance-based seeds (no nuclei)".format(len(coords)))

        markers = np.zeros(distance.shape, dtype=int)
        for i, coord in enumerate(coords, start=1):
            markers[coord[0], coord[1]] = i

        markers = binary_dilation(markers > 0, disk(2))
        markers = label(markers)

    # Apply watershed
    if markers.max() > 0:
        cell_label = watershed(-distance, markers, mask=combined_mask)
        print("    Watershed completed: {} initial regions".format(cell_label.max()))
    else:
        print("    Warning: No watershed seeds detected, using simple labeling")
        cell_label = label(combined_mask)

    # Stage 4: Filter by size and circularity
    props = regionprops(cell_label, intensity_image=img)
    filtered_label = np.zeros_like(cell_label)
    cell_id = 0
    ...
    for prop in props:
        ...
        cell_id += 1
        filtered_label[cell_label == prop.label] = cell_id
        cell_intensities[cell_id] = prop.mean_intensity

    cell_label = filtered_label
    n_cells = cell_id

    # Stage 5: Optional dilation
    ...
    # Stage 6: Match nuclei to cells
    if nuc_label is None:
        nuc_label = np.zeros_like(cell_label)
        print("  Watershed segmentation: {} cells detected (no nuclei)".format(n_cells))
    else:
        ...
        print("  Watershed segmentation: {} cells, {} nuclei".format(
...
def process_fov(fov, path_input, path_output_base, channels, pipeline_params, has_dapi=True):
    ...
    # pol-env colocalization
    pol_env_coloc_pol, pol_env_coloc_env, _ = safe_colocalization(pol_coords, env_coords, voxel_size)
    # pol-nef colocalization
    pol_nef_coloc_pol, pol_nef_coloc_nef, _ = safe_colocalization(pol_coords, nef_coords, voxel_size)
    # env-nef colocalization
    env_nef_coloc_env, env_nef_coloc_nef, _ = safe_colocalization(env_coords, nef_coords, voxel_size)

    print("Colocalization: pol-env={}, pol-nef={}, env-nef={}".format(
        pol_env_coloc_pol.shape[0], pol_nef_coloc_pol.shape[0], env_nef_coloc_env.shape[0]
    ))

    # Classify spots: US (unspliced), SS (singly spliced), MS (multiply spliced)
    # US: pol only (not colocalized with env or nef)
    # SS: pol + env (not colocalized with nef)
    # MS: pol + env + nef (all three colocalized)
    print("Classifying splicing patterns...")

    # Convert to structured arrays for set operations
    pol_struct = spots_to_structured(pol_coords)
    env_struct = spots_to_structured(env_coords)
    nef_struct = spots_to_structured(nef_coords)

    pol_env_struct = spots_to_structured(pol_env_coloc_pol) if pol_env_coloc_pol.size > 0 else spots_to_structured(
        np.empty((0, 2), dtype=np.int64))
    pol_nef_struct = spots_to_structured(pol_nef_coloc_pol) if pol_nef_coloc_pol.size > 0 else spots_to_structured(
        np.empty((0, 2), dtype=np.int64))

    # US: pol spots not in pol_env and not in pol_nef
    pol_in_env = np.isin(pol_struct, pol_env_struct)
    pol_in_nef = np.isin(pol_struct, pol_nef_struct)
    US_mask = ~pol_in_env & ~pol_in_nef
    US_spots = pol_coords[US_mask]

    # SS: pol spots in pol_env but not in pol_nef
    SS_mask = pol_in_env & ~pol_in_nef
    SS_spots = pol_coords[SS_mask]

    # MS: pol spots in both pol_env and pol_nef
    MS_mask = pol_in_env & pol_in_nef
    MS_spots = pol_coords[MS_mask]

    print("Classification: US={}, SS={}, MS={}".format(US_spots.shape[0], SS_spots.shape[0], MS_spots.shape[0]))
