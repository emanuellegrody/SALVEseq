...
# Usage: $0 <csv_file> <fastq_path> <transcriptomepath> <outputpath> <chemistry_version> [extra cellranger flags...]
...
#
# Extra flags (from $6 onward) are passed directly to cellranger count.
# Example:
#   $0 samples.csv rawData/fastq genomes/GRCm39 /output v3 --include-introns=false
#
# NOTE: cellranger flags use '=' with no spaces, e.g. --include-introns=false

if [ "$#" -lt 5 ]; then
    echo "Usage: $0 <csv_file> <fastq_path> <transcriptomepath> <outputpath> <v3|v4> [extra cellranger flags...]"
    exit 1
fi
...
if [ "${#extra_flags[@]}" -gt 0 ]; then
    echo "Extra cellranger flags: ${extra_flags[*]}"
fi
...
tail -n +2 "$csv_file" | tr -d '\r' | while IFS=',' read -r -a fields; do
    ...
    # Determine project from CSV column if present
    project=""
    ...
    echo "Submitting: sample=$sample_name chemistry=$chemistry project=${project:-none} extra_flags=${extra_flags[*]:-none}"
    sbatch "${script_dir}/mkcounts.sh" "$sample_name" "$fastq_path" "$transcriptomepath" "$outputpath" "$chemistry" "$project" "${extra_flags[@]}"
done
