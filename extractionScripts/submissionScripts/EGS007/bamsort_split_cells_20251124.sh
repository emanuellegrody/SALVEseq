...
CELLIDS_FILE="/projects/b1042/GoyalLab/egrody/extractedData/EGS016/singleCell/Seurat/UMAPcoords/D13UMAP_coords.csv"
OUTPUT_ROOT="/projects/b1042/GoyalLab/egrody/extractedData/EGS007/longRead/bamsort/split/"
...
# Create temporary file with cell IDs (one per line)(handles quoted fields)
TEMP_CELLIDS=$(mktemp)
awk -F',' '
NR==1 {
    for(i=1;i<=NF;i++) {
        gsub(/^"|"$/, "", $i)  # Remove quotes from headers
        if($i=="cellID") col=i
    }
    next
}
{
    gsub(/^"|"$/, "", $col)  # Remove quotes from cell ID values
    print $col
}' "${CELLIDS_FILE}" > "${TEMP_CELLIDS}"

# Validate cell_ids file is not empty
CELLID_COUNT=$(wc -l < "${TEMP_CELLIDS}")
if [[ ${CELLID_COUNT} -eq 0 ]]; then
    echo "ERROR: No cellIDs extracted from ${CELLIDS_FILE}" | tee -a "${SUMMARY_FILE}"
    echo "Check that the CSV file has a 'cellID' column header" | tee -a "${SUMMARY_FILE}"
    rm "${TEMP_CELLIDS}"
    exit 1
fi
...
python3 "${SCRIPT_PATH}" "${INPUT_BAM}" "${TEMP_CELLIDS}" "${OUTPUT_ROOT}" --file-input --prefix "cell" > "${LOG_FILE}" 2>&1
