...
#SBATCH --mem=15G
#SBATCH -t 2:00:00
#SBATCH --output=logs/bamsort-split.%j.txt
#SBATCH --error=logs/bamsort-split.%j.err
#SBATCH --verbose
...
SAMPLES_CSV="/projects/b1042/GoyalLab/egrody/extractionScripts/submissionScripts/GEX_samples_v5.csv"
INPUT_END="/outs/possorted_genome_bam.bam"
OUTPUT_ROOT="/projects/b1042/GoyalLab/egrody/extractedData/EGS016/SALVE/bamsort/split/"
...
# Using tail -n +2 to skip the header row
SAMPLES=($(tail -n +2 "$SAMPLES_CSV" | cut -d, -f2))
DATASETS=($(tail -n +2 "$SAMPLES_CSV" | cut -d, -f1))
FOLDERS=($(tail -n +2 "$SAMPLES_CSV" | cut -d, -f3))
...
for i in "${!SAMPLES[@]}"; do
    ...
    INPUT_PATTERN="${FOLDER}*${DATASET}${INPUT_END}"
    
    # Expand the glob pattern to get actual file path
    # This converts the wildcard pattern into the actual matching file(s)
    INPUT_FILES=( $INPUT_PATTERN )
    
    # Check if exactly one file was matched
    if [ ${#INPUT_FILES[@]} -eq 0 ]; then
        ...
        echo "WARNING: Multiple files found matching pattern: ${INPUT_PATTERN}"
        echo "Using first match: ${INPUT_FILES[0]}"
    fi
    
    INPUT_COMPLETE="${INPUT_FILES[0]}"
