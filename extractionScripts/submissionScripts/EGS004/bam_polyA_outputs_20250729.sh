...
# Update parameters
PYTHON_SCRIPT="/projects/b1042/GoyalLab/egrody/extractionScripts/Python/bam_polyA_outputs.py"
...
PYTHON_ARGS="--output_example_sequences 200"
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "${OUTPUT_PREFIX}_example_sequences.txt" ]; then
        echo "  - ${OUTPUT_PREFIX}_example_sequences.txt (example sequences from mapped/paired reads)"
    fi
