...
Usage:
    python3 bamsort_splice_liver.py input.bam output.csv [--include-all-splicing]
...
def find_splice_junctions(read, min_intron_size=50, min_match_size=20):
    """
...
def main(bam_file, output_csv, reference=None,
         min_intron_size=50, min_match_size=20):
    # Validate inputs
    ...
    # If a reference is specified, only iterate over that contig.
    # This is both a filter and a performance optimization -- fetching
    # a single chromosome avoids scanning the entire BAM.
    if reference:
        refs = set(bam.references)
        if reference not in refs:
            print(f"WARNING: reference '{reference}' not in BAM header.")
            print(f"Available (first 10): {list(bam.references[:10])}")
            print("Falling back to scanning all references.")
            read_iter = bam
            reference = None  # disable filtering
        else:
            read_iter = bam.fetch(reference)
    else:
        read_iter = bam
    ...
    try:
        with open(output_csv, "w", newline="") as csvfile:
            ...
            for read in read_iter:
                ...
                # Reference filtering (only if specified)
                if reference and read.reference_name != reference:
                    reads_wrong_ref += 1
                    continue
    ...
    if reference:
        print(f"Reads from other references: {reads_wrong_ref}")
...
Examples:
  ...
  python3 bamsort_splice.py input.bam output.csv --include-all-splicing
    ...
    parser.add_argument("--reference", type=str, default=None,
                        help="Reference/chromosome to filter to (e.g. mac239, chr6). "
                             "If omitted, all references are scanned.")
    ...
    print(f"Reference filtering: {args.reference if args.reference else 'all references'}")
    print(f"Minimum intron size: {args.min_intron}bp")
    print(f"Minimum match size: {args.min_match}bp")
    print("-" * 60)

    try:
        main(args.bam_file, args.output_csv, args.reference,
