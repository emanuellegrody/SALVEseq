"""
BAM Regional UMI-Cell Splitter

Main Functions:
- Processes entire BAM file to classify reads as "inside" or "outside" a target region
- Extracts UMI and cell barcode information from high-quality reads (xf:i:25 or xf:i:17)
- Counts unique UMI-cell combinations separately for each region

Usage:
    python bamsort_split.py input.bam chr1 1000000 1001000 inside.csv outside.csv

Input Requirements:
- BAM file with CB (cell barcode), UB (UMI), and xf (quality) tags
- Genomic coordinates: chromosome, start position, end position
- Two output CSV file paths (inside and outside region)

Output:
- inside.csv: UMI-cell combinations from reads within the specified region
- outside.csv: UMI-cell combinations from reads outside the specified region
- Both CSV files contain columns: cellID, UMI, reads (count)
- Console statistics comparing inside vs. outside region metrics
